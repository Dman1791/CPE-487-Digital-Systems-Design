...
ARCHITECTURE Behavioral OF hexcalc IS
	...
	SIGNAL do_add : STD_LOGIC; -- 1 if addition should be done, 0 if subtraction instead
			...
			CASE pr_state IS -- depending on present state...
				WHEN ENTER_ACC => -- waiting for next digit in 1st operand entry
					...
					ELSIF bt_plus = '1' THEN
						nx_state <= START_OP;
						do_add <= '1'; --set addition flag
					ELSIF bt_nd = '1' THEN
					    nx_state <= START_OP;
					    do_add <= '0'; --set addition flag
					ELSE
						nx_state <= ENTER_ACC;
					END IF;
				...
				WHEN ENTER_OP => -- waiting for next digit in 2nd operand
					display <= operand;
					IF bt_eq = '1' THEN
					    if (do_add = '1') then --check addition flag, do subtraction instead if 0
						nx_acc <= acc + operand;
						else
						nx_acc <= acc - operand;
						end if;
